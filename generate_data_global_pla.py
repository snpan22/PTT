#!/usr/bin/env python
"""
generate_data_global_pla.py

PLA-LiDAR constrained variant of generate_data_global.py. Identical pipeline
with three additional physical constraints applied to every spoofed frame
(current and historical) before raycasting:
...
Attacker model. The attacker is a physically distinct entity from the
phantom object. For each spoof run, an offset is sampled in the anchor-
frame ego-aligned axes (x = forward, y = left, z = up) and added to the
phantom centre; the result is then transformed to world coordinates using
the anchor ego pose. The attacker is static in the world frame during the
spoof run, matching the global_fixed threat model in ATLAS. For each lag
frame of each timestep's 4-frame input window, the attacker world position
is transformed into that frame's ego coordinates and passed into the cone /
range-noise / jitter mask alongside the corresponding spoof points.

Default offset ranges model a roadside spoofer. Lateral offset magnitude is
sampled from U(PLA_ATTACKER_LAT_MIN_M, PLA_ATTACKER_LAT_MAX_M) with sign
drawn uniformly from {-1, +1}; longitudinal offset is sampled from
U(PLA_ATTACKER_LON_MIN_M, PLA_ATTACKER_LON_MAX_M); vertical is fixed at
PLA_ATTACKER_Z_OFFSET_M (slightly below phantom centroid, roughly ground
level). Setting all three ranges to zero reproduces the co-located baseline.

Per-frame PLA retention counts land in the saved dataset dicts. Each spoof
run's sampled attacker offset and world position are recorded in the
spoof_annotation; an aggregate summary is written to each segment's
annotations pkl.
"""
import os
import sys
...
# ---------------------------------------------------------------------------
# Attacker offset sampling parameters
# ---------------------------------------------------------------------------
# Offset is sampled in the anchor-frame ego-aligned axes:
#   x = forward (toward direction of ego motion)
#   y = left    (signed, sign drawn uniformly from +/-1)
#   z = up
# Defaults model a roadside spoofer close to phantom longitudinally, offset
# laterally by ~1.5-3.5 m (the shoulder of a typical lane relative to a
# phantom in an adjacent lane) and slightly below phantom centroid height
# (phantom centroid ~0.9 m above ground for a sedan; attacker device near
# ground level). Set all three ranges to zero to reproduce the co-located
# baseline attacker.
PLA_ATTACKER_LAT_MIN_M = 1.5
PLA_ATTACKER_LAT_MAX_M = 3.5
PLA_ATTACKER_LON_MIN_M = -1.0
PLA_ATTACKER_LON_MAX_M = 1.0
PLA_ATTACKER_Z_OFFSET_M = -0.3

def safe_ratio(a, b):
    return a / b if b > 0 else 0.0
def sample_attacker_offset_anchor_ego(rng):
    """
    Draw one (dx, dy, dz) offset in the anchor ego frame's local axes.
    Lateral magnitude is uniform in [LAT_MIN, LAT_MAX]; sign uniform on
    {-1, +1}. Longitudinal is uniform in [LON_MIN, LON_MAX]. Vertical is
    fixed at PLA_ATTACKER_Z_OFFSET_M.

    Returns
    -------
    offset_anchor_ego : (3,) ndarray of float64
    """
    lat_mag = rng.uniform(PLA_ATTACKER_LAT_MIN_M, PLA_ATTACKER_LAT_MAX_M)
    lat_sign = 1.0 if rng.uniform() < 0.5 else -1.0
    lon = rng.uniform(PLA_ATTACKER_LON_MIN_M, PLA_ATTACKER_LON_MAX_M)
    return np.array([lon, lat_sign * lat_mag, PLA_ATTACKER_Z_OFFSET_M],
                    dtype=np.float64)
...
def place_trace(traces, dataset_mf, perturbation, window_seg_min, window_seg_max, anchor_pose=None):
    """
    For global_position_fixed:
      anchor_pose is the (4,4) ego pose of the LAST frame in the spoof run.
      The trace is placed in ego-relative coords of that anchor frame,
      then converted to world/global coordinates. Every historical frame
      will then transform from global back into its own ego frame inside
      process_4frames.

    Also samples an attacker offset in the anchor ego frame, adds it to the
    phantom centre (in anchor ego coords), and converts to world frame using
    the same anchor_pose. The attacker world position is returned alongside
    the other outputs and is attached to the run's spoof_annotation.
    """
    rng = np.random.default_rng()
    ...
    # --- Convert to global coordinates using anchor pose ---
    assert anchor_pose is not None, "anchor_pose required for global_position_fixed"

    # points: rotate by anchor rotation, then translate
    trace_global_xyz = (front_near_trace @ anchor_pose[:3, :3].T) + anchor_pose[:3, 3]
    spoof_points = np.hstack((trace_global_xyz, trace['points'][:, 3:5]))

    # box centre: same rotation + translation. Keep a copy of the phantom
    # centre in anchor ego coords before globalising, so we can place the
    # attacker relative to it in that frame.
    phantom_xyz_anchor_ego = np.array([front_near_box[0],
                                       front_near_box[1],
                                       front_near_box[2]], dtype=np.float64)

    box_xyz_global = (phantom_xyz_anchor_ego @ anchor_pose[:3, :3].T) + anchor_pose[:3, 3]
    front_near_box[0] = box_xyz_global[0]
    front_near_box[1] = box_xyz_global[1]
    front_near_box[2] = box_xyz_global[2]
    front_near_box[6] = front_near_box[6] + np.arctan2(anchor_pose[1, 0], anchor_pose[0, 0])

    # --- Sample attacker offset in anchor ego frame and transform to world --
    # attacker_offset_anchor_ego = sample_attacker_offset_anchor_ego(rng)
    #attacker_xyz_anchor_ego = phantom_xyz_anchor_ego + attacker_offset_anchor_ego

    phantom_width = centered_box[4] # x, y, z, dx, dy, dz, ...
    sign = 1.0 if phantom_xyz_anchor_ego[1] >= 0 else -1.0
    clearance_from_side = 1.1  # meters
    lateral_offset = 0.5 * phantom_width + clearance_from_side

    attacker_offset_anchor_ego = np.array([3.0, sign * lateral_offset, -0.3])
    attacker_xyz_anchor_ego = phantom_xyz_anchor_ego + attacker_offset_anchor_ego

    attacker_xyz_global = (attacker_xyz_anchor_ego @ anchor_pose[:3, :3].T) + anchor_pose[:3, 3]

    annotation = {
        'window': [window_seg_min, window_seg_max],
        'perturbation': perturbation,
        # attacker bookkeeping (all in world frame unless noted)
        'attacker_offset_anchor_ego': attacker_offset_anchor_ego.tolist(),
        'attacker_pos_global': attacker_xyz_global.tolist(),
        'phantom_pos_anchor_ego': phantom_xyz_anchor_ego.tolist(),
        'phantom_pos_global': box_xyz_global.tolist(),
    }

    return spoof_points, annotation, front_near_box, attacker_xyz_global
...
def process_4frames(dataset_mf,
    ...
    # attacker map values are (3,) world-frame positions; shallow-copy is fine
    ...
    for i, idx in enumerate(indices):
        ...
        if (target_frames_copy[idx] is not None):
            ...
            # Attacker has its own world-frame position (sampled per spoof run
            # in place_trace as phantom + offset in anchor ego axes). Transform
            # that world position into the lag-i ego frame so the cone is
            # centered on the direction the ego would observe the attacker
            # from at this frame's pose.
            attacker_pos_world = target_frames_attacker_copy[idx]
            if attacker_pos_world is None:
                ...
                att_hom = np.array([attacker_pos_world[0],
                                    attacker_pos_world[1],
                                    attacker_pos_world[2], 1.0])
                att_ego = att_hom @ np.linalg.inv(pose_lag_i_frame).T
                attacker_pos_ego = att_ego[:3]
...
def _finalize_segment_pla_summary(totals):
    ...
    summary['constraint_params'] = {
        ...
        # 'attacker_lat_min_m': PLA_ATTACKER_LAT_MIN_M,
        # 'attacker_lat_max_m': PLA_ATTACKER_LAT_MAX_M,
        # 'attacker_lon_min_m': PLA_ATTACKER_LON_MIN_M,
        # 'attacker_lon_max_m': PLA_ATTACKER_LON_MAX_M,
...
def main():
    ...
    log.info(
        "PLA constraints enabled: cone=+/-%.1f deg (az) x +/-%.1f deg (el), "
        "jitter_std=%.2f deg, range_noise base=%.3f m slope=%.4f m/deg",
        PLA_CONE_HALF_AZ_DEG, PLA_CONE_HALF_EL_DEG,
        PLA_JITTER_STD_DEG,
        PLA_RANGE_NOISE_BASE_M, PLA_RANGE_NOISE_SLOPE_M_PER_DEG,
    )
    log.info(
        "PLA attacker offset (anchor ego frame): lateral |y| in [%.2f, %.2f] m "
        "(sign uniform +/-), longitudinal x in [%.2f, %.2f] m, vertical z = %.2f m",
        PLA_ATTACKER_LAT_MIN_M, PLA_ATTACKER_LAT_MAX_M,
        PLA_ATTACKER_LON_MIN_M, PLA_ATTACKER_LON_MAX_M,
        ...
        for (frame, batch) in enumerate(test_loader_mf):
            ...
            if current_segment is None:
                ...
                for run in runs_global:
                    ...
                    anchor_global_idx = max(subset_g.keys())
                    anchor_local_idx = global_to_segment[anchor_global_idx][1]
                    anchor_pose = seg_localidx_to_pose[(current_segment, anchor_local_idx)]

                    trace, spoof_annotation, box, attacker_global = place_trace(
                        traces, dataset_mf, 'global_position_fixed',
                        window_min, window_max, anchor_pose=anchor_pose
                    )
                    ...
                    for key, key_a, key_t in zip(subset_g.keys(),
                        ...
                        subset_g_att[key_t] = attacker_global.copy()
