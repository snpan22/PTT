...
def process_4frames(dataset_mf,
    ...
    for i, idx in enumerate(indices):
        ...
        if(target_frames[idx] is not None):
            ...
            spoof_points = target_frames[idx]
...
def generate_timing_classification(target_frames):
    classification = []
    for i in target_frames.keys():
        if i<=31 or i == 95 or i == 159:
            classification.append(0)
        elif i == 32 or i == 96 or i==160:
            classification.append(1)
        elif (i>=33 and i<=63) or (i>=97 and i <=127) or (i>=161 and i <= 191):
...
def place_trace(traces, dataset_mf, perturbation, window_seg_min, window_seg_max):
    ...
    relative_position_offset= np.array([rng.uniform(7, 10), rng.uniform(-1, 1) ])
    ...
    annotation = {
        ...
        'spoof_points': spoof_points
    }
    
    return spoof_points, annotation
        ...
        for (frame,batch)  in enumerate(test_loader_mf):
            ...
            if current_segment is None:
                ...
                for run in runs_global:
                    ...
                    trace, spoof_annotation = place_trace(traces, dataset_mf, 'relative_position_fixed', window_min, window_max)
                    
                    for key in subset_g.keys():
                        subset_g[key] = trace
            ...
            if(segment != current_segment):
                ...
                segment_annos['target_frames'] = target_frames_seg[current_segment]
