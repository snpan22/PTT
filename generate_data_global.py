...
def place_trace(traces, dataset_mf, perturbation, window_seg_min, window_seg_max, anchor_pose=None):
    ...
    relative_position_offset = np.array([rng.uniform(9, 11), rng.uniform(-3, 3)])
    ...
    # box centre: same rotation + translation
    box_xyz_local = np.array([front_near_box[0], front_near_box[1], front_near_box[2]])
    box_xyz_global = (box_xyz_local @ anchor_pose[:3, :3].T) + anchor_pose[:3, 3]
...
#                 spf_cur = spoof_points_homo
            ...
            if perturbation == "global_position_fixed":
                # rotate+translate box centre from global to lag-i ego frame
                ...
                # heading: subtract lag-i yaw
                gt_box_for_frame[6] = (gt_box_for_frame[6]
                                       - np.arctan2(pose_lag_i_frame[1, 0],
                                                    pose_lag_i_frame[0, 0]))
            frame_points_spoof_rc, gt_box, n_spoof_r, n_spoof_k = spoof_frame(
                spf_cur_lag_i, points_lag_i_frame_in_lag_i_coords_full, gt_box_for_frame
            )
...
def main():
    ...
    # (segment, local_idx) -> ego pose (4,4) — needed to compute anchor_pose
        ...
        for (frame,batch)  in enumerate(test_loader_mf):
            ...
            if current_segment is None:
                ...
                for run in runs_global:
                    ...
                    # anchor pose = ego pose of LAST frame in this spoof run
                    ...
                    trace, spoof_annotation, box = place_trace(
