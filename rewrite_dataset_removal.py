    ...
    try:
        for(frame,batch)  in enumerate(test_loader_mf):
            ...
            if current_segment != segment:
                ...
                segment_frames = []
                # load spoof dataset
                seg_annos_file = f"{args.annos_dir}/{current_segment}_a.pkl"
                with open(seg_annos_file, "rb") as f:
            ...
            if(seg_dataset[seg_idx]['spoof_gt'] is not None):
                if(seg_annos['spoof_annos'][seg_idx]['active_this_frame']):   
                    dict_mf_mod['spoof_gt'] = seg_dataset[seg_idx]['spoof_gt']
                else:
                    dict_mf_mod['spoof_gt'] = None
            else:
                dict_mf_mod['spoof_gt'] = None
